import styles from "./frame-component11.module.css";

const FrameComponent11 = () => {
  return (
    <div className={styles.frameParent}>
      <div className={styles.frameWrapper}>
        <div className={styles.frameGroup}>
          <div className={styles.contentParent}>
            <div className={styles.content}>
              <img
                className={styles.image48Icon}
                loading="lazy"
                alt=""
                src="/image-48@2x.png"
              />
              <img
                className={styles.image53Icon}
                loading="lazy"
                alt=""
                src="/image-53@2x.png"
              />
              <img
                className={styles.image50Icon}
                loading="lazy"
                alt=""
                src="/image-50@2x.png"
              />
              <img
                className={styles.image47Icon}
                loading="lazy"
                alt=""
                src="/image-47@2x.png"
              />
              <img
                className={styles.placeholderImage}
                loading="lazy"
                alt=""
                src="/placeholder--image@2x.png"
              />
            </div>
            <img
              className={styles.image51Icon}
              loading="lazy"
              alt=""
              src="/image-51@2x.png"
            />
          </div>
          <img
            className={styles.image57Icon}
            loading="lazy"
            alt=""
            src="/image-57@2x.png"
          />
        </div>
      </div>
      <div className={styles.frameContainer}>
        <div className={styles.cab116853459251Parent}>
          <img
            className={styles.cab116853459251Icon}
            loading="lazy"
            alt=""
            src="/647456850cab1-1685345925-1@2x.png"
          />
          <img
            className={styles.image52Icon}
            loading="lazy"
            alt=""
            src="/image-52@2x.png"
          />
        </div>
      </div>
      <div className={styles.f5b16825306791Parent}>
        <img
          className={styles.f5b16825306791Icon}
          loading="lazy"
          alt=""
          src="/6449617741f5b-1682530679-1@2x.png"
        />
        <img
          className={styles.ddae6316853463331Icon}
          loading="lazy"
          alt=""
          src="/6474581ddae63-1685346333-1@2x.png"
        />
      </div>
      <div className={styles.d7557be16825307751Parent}>
        <img
          className={styles.d7557be16825307751Icon}
          loading="lazy"
          alt=""
          src="/644961d7557be-1682530775-1@2x.png"
        />
        <img
          className={styles.faf64516853463991Icon}
          loading="lazy"
          alt=""
          src="/6474585faf645-1685346399-1@2x.png"
        />
      </div>
    </div>
  );
};

export default FrameComponent11;
